            ...
            for yhe in hegrid:
                ...
                if basis == 'sp':
                    ...
                    res1 = mixtures_eos.get_p_rhot(xarr, yarr, hearr, zgrid, \
                                                    hhe_eos=xy_eos, alg='root', z_eos=z_eos, hg=hg)
...
svals_sp = np.arange(2.0, 9.1, 0.1)
logpvals_sp = np.arange(6, 14.1, 0.1)
yvals_sp = np.arange(0.05, 0.9, 0.05)
zvals_sp = np.arange(0, 0.92, 0.02)
...
logt_res_sp, logrho_res_sp = inversion_z(svals_sp, logpvals_sp, yvals_sp, zvals_sp, \
                                        basis='sp', xy_eos='cms', z_eos='aqua', hg=True)

np.save('eos/cms/sp_base_z_aqua_cms_hg_updated_dense.npy', [logrho_res_sp, logt_res_sp])

# svals_srho = np.arange(4.0, 9.51, 0.01)
# logrhovals_srho = np.linspace(-5.0, 2.0, 100)
# yvals_srho = np.arange(0.05, 0.99, 0.01)
# zvals_srho = np.arange(0, 0.99, 0.005)
...
# #np.save('eos/cms/srho_base_z_aqua_extended_hg.npy', [logp_res_srho, logt_res_srho])
